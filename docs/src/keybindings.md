...
| Paperless documents | Type to filter the loaded batch by title (substring, client-side); `↑`/`↓` move the highlight. `Enter` runs a **server-side full-text search** with the current text and replaces the list. Per-row `Open` opens the document's paperless web page in the browser; `Download` saves the PDF into the active pane's folder. `Esc` dismisses. Only enabled once `paperless.url` + `paperless.token` are set (see [configuration](./configuration.md#paperless)). |
